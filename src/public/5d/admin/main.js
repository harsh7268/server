...
function messNewJoin2(datas) {
    ...
    datas.map((data) => {
        ...
        <div class="direct-chat-text" style="background-color: #1eb93d"> Tham gia ${(isNumber(list_join2)) ? "( " + list_join2 + " )" : (list_join2 == 'b') ? 'Lớn' : (list_join2 == 's') ? 'Nhỏ' : (list_join2 == 'c') ? 'Chẵn' : 'Lẻ'} ${money} </div>
        `; 
    });
